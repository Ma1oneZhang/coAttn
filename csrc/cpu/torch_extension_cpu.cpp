...
void extend_attention_cpu(
    ...
    double sm_scale,
    double logit_cap,
    const std::optional<at::Tensor>& logsumexp_extend = std::nullopt);
...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      ...
      "extend_start_loc, int max_len_extend, float sm_scale, float logit_cap, Tensor? logsumexp_extend=None) -> ()");
  m.impl("extend_attention_cpu", torch::kCPU, &extend_attention_cpu);

  // attention_buffer_only
  m.def(
      "attention_buffer_only_cpu(Tensor q_extend, Tensor k_buffer, Tensor v_buffer, Tensor(a!) o_extend, "
      "Tensor req_to_token, Tensor req_pool_indices, Tensor seq_lens, int max_context_len, float sm_scale, "
      "float logit_cap) -> Tensor");
  m.impl("attention_buffer_only_cpu", torch::kCPU, &attention_buffer_only_cpu);
