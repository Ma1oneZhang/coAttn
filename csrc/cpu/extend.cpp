...
void extend_attention_kernel_impl(
    ...
    float* __restrict__ logsumexp_extend,  // Add logsumexp parameter
    int batches,
    int num_heads,
    int num_heads_kv,
    int head_size,
    ...
    for (int i = begin; i < end; ++i) {
      ...
      for (int n = 0; n < seq_len_prefix; n += BLOCK_N) {
        ...
        pack_vnni<scalar_t, index_t>(
            ...
            /*    indice */ req_to_token + req_pool_id * max_context_len + n,
            /*     N  */ n_size,
            /*     K  */ head_size,
            /* ld_src */ k_strideN,
            /* ld_dst */ BLOCK_N);
      ...
      for (int row = 0; row < m_size; ++row) {
        ...
        if (logsumexp_extend != nullptr) {
          int global_row = seq_extend_start_loc + m + row;
...
void extend_attention_cpu(
    ...
    double sm_scale,
    double logit_cap,
    const std::optional<at::Tensor>& logsumexp_extend) {
  RECORD_FUNCTION(
  ...
  // Check logsumexp if provided
  if (logsumexp_extend.has_value()) {
    CHECK_INPUT(logsumexp_extend.value());
  }
  ...
  AT_DISPATCH_REDUCED_FLOATING_TYPES(q_extend.scalar_type(), "extend_attention_kernel", [&] {
    AT_DISPATCH_INDEX_TYPES(index_dtype, "extend_attention_indices", [&] {
      extend_attention_kernel_impl<scalar_t, index_t, BLOCK_M, BLOCK_N>(
          ...
          buffer.data_ptr(),
          logsumexp_extend.has_value() ? logsumexp_extend.value().data_ptr<float>() : nullptr,
          num_seqs,
          num_heads,
          num_heads_kv,
          head_size,
