...
void attention_buffer_only_kernel_impl(
    scalar_t* __restrict__ o_extend,
    float* __restrict__ logsumexp_extend,
  ...
  at::parallel_for(0, batches * num_heads * MB, 0, [&](int begin, int end) {
    ...
    for (int i = begin; i < end; ++i) {
      ...
      // get query
      const scalar_t* __restrict__ q_ptr = q_extend + m * q_strideM + head_id * q_strideH;
      ...
      scalar_t* __restrict__ out_ptr = o_extend + m * o_strideM + head_id * o_strideH;
      for (int row = 0; row < m_size; ++row) {
        float s = 1 / s_prime[row];
        copy_stub<scalar_t>(out_ptr + row * o_strideM, v_prime + row * head_size_v, s, head_size_v);

        // Store logsumexp
        int global_row = m + row;
        if (global_row < max_extend_len) {
          logsumexp_extend[global_row * num_heads + head_id] = m_prime[row] + std::log(s_prime[row]);
        }
      }
...
// attention_buffer_only_cpu: computes attention for q_extend only with k_buffer/v_buffer
// Returns logsumexp tensor
//
// q_extend: [num_tokens, num_heads, head_size]
// k_buffer: [max_total_num_tokens, num_heads_kv, head_size]
// v_buffer: [max_total_num_tokens, num_heads_kv, head_size]
...
// seq_lens: [num_seqs] int64 (prefix lengths)
// logsumexp: [num_tokens, num_heads] float32 (output)
at::Tensor attention_buffer_only_cpu(
    at::Tensor& q_extend,
    at::Tensor& k_buffer,
    at::Tensor& v_buffer,
    at::Tensor& o_extend,
    ...
    double logit_cap) {
  RECORD_FUNCTION(
      "sgl-kernel::attention_buffer_only_cpu",
  ...
  int num_tokens = q_extend.size(0);
  ...
  TORCH_CHECK(
      ...
      "attention_buffer_only: expect req_to_token to be int32 or int64, got ",
      index_dtype);
  TORCH_CHECK(seq_lens.scalar_type() == at::kLong, "attention_buffer_only: expect seq_lens to be int64, got ", seq_lens.scalar_type());
  TORCH_CHECK(
      req_pool_indices.scalar_type() == at::kLong,
      "attention_buffer_only: expect req_pool_indices to be int64, got ",
  ...
  // Create logsumexp tensor
  auto logsumexp = at::empty({num_tokens, num_heads}, q_extend.options().dtype(at::kFloat));

  AT_DISPATCH_REDUCED_FLOATING_TYPES(q_extend.scalar_type(), "attention_buffer_only_kernel", [&] {
    AT_DISPATCH_INDEX_TYPES(index_dtype, "attention_buffer_only_indices", [&] {
      attention_buffer_only_kernel_impl<scalar_t, index_t, BLOCK_M, BLOCK_N>(
